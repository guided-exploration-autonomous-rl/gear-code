...
import wandb
wandb_run = wandb.init("autoregressive", name=f"trials")

model = autoregressive_model.AutoRegressiveModel(3, 3, 1024, 3, num_buckets = 20, ac_low = np.array([-3, -2, -1]), ac_high = np.array([3, 2, 1])).cuda()
...
def train_autoregressive_model(batch_size=4096, num_epochs=30000, freq_print = None):
    """
    ...
    for epoch in range(num_epochs):
        ...
        loss = -torch.mean(model.log_prob(states_1, states_2))
        loss.backward()
        optimizer.step()
...
def forward_autoregressive_model(s1, s2):
    ...
    s1 = np.clip(bucket_idx, 0, num_buckets - 1)
    
    states_1 = torch.Tensor(s1).cuda()
    states_2 = torch.Tensor(s2).cuda()

    with torch.no_grad():
        return model.log_prob(states_1, states_2).detach().cpu().numpy()
...
plt.imsave("autoregressive_res.png")
wandb.log({"autoregressive_model_res":wandb.Image(plt)})
r_val = np.array([query(starts[i], states[i]) for i in range(len(starts))])
plt.clf()
plt.cla()
plt.scatter(states[:, 0], states[:, 1], c=r_val, cmap=cm.jet)
plt.imsave("query_res.png")
wandb.log({"query_res":wandb.Image(plt)})
