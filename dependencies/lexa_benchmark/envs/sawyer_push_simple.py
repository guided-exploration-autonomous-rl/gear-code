...
import numpy as np
from dependencies.lexa_benchmark.envs.base_envs import BenchEnv
...
push_configs = {
    ...
    'fixed_start': dict(
            ...
            puck_low=(-0.20, 0.50),
            puck_high=(0.20, 0.70),
            reset_low=(-0.25,  0.50,-0.17,  0.53), #0.08, 0.53),
            reset_high=( -0.25, 0.5,-0.1, 0.6),#0.25, 0.60),
            goal_low=(0.08, 0.53, 0.08, 0.55),#-0.25,  0.50),
            goal_high=( 0.25, 0.60,0.25, 0.65),#-0.08, 0.53),
            fix_reset=False,
            sample_realistic_goals=True,
            invisible_boundary_wall=True,
            reward_type='puck_and_hand',
...
class SawyerPushGoalEnv(BenchEnv):
    def __init__(self, action_repeat=1, use_goal_idx=False, log_per_goal=False,  control_mode='end_effector', width=64,  fixed_start=True, fixed_goal=False, images=False, image_kwargs=None):
    ...
    def compute_shaped_distance(self, achieved_state, goal):
        ...
        subgoal = np.array([(0, 0.65), (0, 0.55)])
        achieved_state_puck = self.observation(achieved_state)[2:4]
        desired_goal_puck = goal[2:4]
        if achieved_state_puck[0] < 0 :
            distance = min(np.linalg.norm(achieved_state_puck-subgoal[0])+np.linalg.norm(subgoal[0]-desired_goal_puck), np.linalg.norm(achieved_state_puck-subgoal[1])+np.linalg.norm(subgoal[1]-desired_goal_puck))
        else:
