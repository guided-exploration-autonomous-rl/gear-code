...
from gear.algo.networks import RewardModelHumanPreferences
import numpy as np
import torch
import time
import wandb
...
from gear.algo import buffer_ddl, networks
...
class BatchRLAlgorithmDDL(BaseRLAlgorithm, metaclass=abc.ABCMeta):
    def __init__(
            ...
            reward_model=None,
            reward_model_buffer=None,
            reward_model_buffer_validation=None,
    ):
        super().__init__(
            trainer,
            exploration_env,
    ...
    def train_reward_model(self,device='cuda'):
        ...
        for epoch in range(self.reward_model_epochs):  # loop over the dataset multiple times
            ...
            state1 = torch.Tensor(achieved_states_1).to(device)
            state2 = torch.Tensor(achieved_states_2).to(device)
        ...
        if prev_losses[0]==prev_losses[-1]:
            ...
            return self.train_reward_model(device)
    ...
    def collect_and_train_reward_model(self):
        ...
        if not self.no_training:
            ...
            losses_reward_model, eval_loss_reward_model = self.train_reward_model('cuda')
        ...
        for epoch in range(self.num_train_loops_per_epoch):
            ...
            for i in range(self.num_trains_per_train_loop):
                ...
                if self.use_oracle:
                    ...
                    goal_tensor = torch.Tensor(np.array([self.goal for _ in range(self.batch_size)])).to('cuda')
                    new_rewards = self.reward_model(tensor_states, self.goal_tensor)
                else:
