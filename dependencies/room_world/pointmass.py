...
class PMEnv(RoomEnv):   
    ...
    def __init__(self,
        ...
        self.goals = np.array([[0.25, 0.25], [0.4, -0.4]])
        self.index = 0
    ...
    def success(self, a, b):
        return self.room.get_shaped_distance(a, b) < 0.05

    def sample_goals(self, batch_size):
        if self.success(self._get_obs()["observation"], self.goals[self.index]):
            self.index = (self.index + 1) % 2    

        goals = self.goals[self.index]
        # else:
