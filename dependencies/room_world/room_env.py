...
class RoomEnv(MujocoEnv, MultitaskEnv, Serializable):
    ...
    def __init__(self,
        ...
        self.goals = np.array([[0.25, 0.25], [0.25, -0.25]])
    ...
    def sample_point(self, config):
        ...
        if config == "bimodal":
            ...
            goal_poses = np.array([[0.25,-0.25], [0.25, 0.25]])
            goal = np.array(goal_poses[idx])
            print("goal", goal)
            return goal
    ...
    def sample_goals(self, batch_size):
        ...
        self._set_goal_marker(goals)

        goals = {
            'desired_goal': goals,
            'state_desired_goal': goals,
        }
        print("sampling goals", goals)
        return goals
