...
class ReplayBuffer:
    ...
    def __init__(self,
        ...
        self._states = np.zeros(
            (buffer_size, max_trajectory_length, env.observation_space.shape[0]*2),
    ...
    def add_trajectory(self, states, actions, length_of_traj=None):
        """
    ...
    def add_multiple_trajectory(self, states, actions, length_of_traj=None):
        """
        Adds a trajectory to the buffer
        ...
        for actions_t, states_t in zip(actions, states):
            self.add_trajectory(np.array(states_t), np.array(actions_t), length_of_traj )
