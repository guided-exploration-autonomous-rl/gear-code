...
class RewardModel(nn.Module):
    def __init__(self, env, obs_dim, fourier=False, normalize=False, layers=[600,600], is_complex_maze=False):
        ...
        print("obs dim", obs_dim, dim, env.observation_space)
    ...
    def forward(self, obs, goal):
        ...
        state = torch.concat([obs, goal], axis=-1).to("cuda:0")
...
class GaussianFCNetwork(Mlp, TorchStochasticPolicy):
    ...
    def forward(self, obs):
        ...
        if self.std is None:
            ...
            std = torch.from_numpy(np.array([self.std, ])).float().to("cuda:0")
...
class StateGoalNetwork(nn.Module):
    def __init__(self, env, dim_out=1, image_size = None, state_embedding=None, continuous_action_space=False, fourier=False, goal_embedding=None, layers=[512, 512], max_horizon=None, freeze_embeddings=False, add_extra_conditioning=False, dropout=0, is_complex_maze= False):
        ...
        if image_size is not None:
            state_dim_in = self.state_embedding(torch.zeros((1, 3, image_size, image_size))).shape[1]
            goal_dim_in = self.goal_embedding(torch.zeros((1, 3, image_size, image_size))).shape[1]
        else:
    ...
    def forward(self, state, goal, horizon=None):
        ...
        if self.is_complex_maze and self.image_size is None:
            goal = goal[:, :2]
...
class DiscreteStochasticGoalPolicy(nn.Module, policy.GoalConditionedPolicy):
    ...
    def act_vectorized(self, obs, goal, horizon=None, greedy=False, noise=0):
        if self.normalize:
            ...
            obs = torch.tensor(obs, dtype=torch.float32).to("cuda:0")
            goal = torch.tensor(goal, dtype=torch.float32).to("cuda:0")
        ...
        return ptu.to_numpy(samples), probs
...
class IndependentDiscretizedStochasticGoalPolicy(nn.Module, policy.GoalConditionedPolicy):
    ...
    def flattened(self, tensor):
        ...
        multipliers = self.granularity ** torch.tensor(np.arange(self.n_dims)).to("cuda:0")
...
class AutoRegressiveModel(nn.Module):
    ...
    def __init__(self, env, action_dim, hidden_dim, hidden_depth):        
        super().__init__()
        self.env = env
        self.obs_dim = env.observation_space.shape
        self.goal_dim = env.observation_space.shape
        self.num_buckets = env.action_space.shape
        self.action_dim = action_dim
        self.eps = 1e-8
        self.trunks = nn.ModuleList([mlp(self.obs_dim + self.goal_dim, hidden_dim, self.num_buckets, hidden_depth)] \
                        + [mlp(self.obs_dim + self.goal_dim + j + 1, hidden_dim, self.num_buckets, hidden_depth) for j in range(action_dim - 1)])
        self.num_dims = action_dim
        self.ac_low = torch.tensor(0).to(device)
        self.ac_high = torch.tensor(env.action_space.shape).to(device)
        self.bucket_size = torch.tensor((self.ac_high - self.ac_low) / self.num_buckets).to(device)
    ...
    def log_prob(self, obs, goal, act):
        ...
        for j in range(self.num_dims):
            ...
            lp = dist.log_prob(ac_discretized[:, 0])
            log_prob += lp
            ac_prev = ac_discretized[:,0]
        return log_prob
