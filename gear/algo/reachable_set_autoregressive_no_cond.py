...
class ReachableSetAutoregressiveNoCond():
    def __init__(self, env, replay_buffer,  env_name,max_diff = 0, size=512, epochs=30000, num_buckets=20, batch_size=4096, width_bin=5, height_bin=5):
        self.env = env
        ...
        if "kitchen" in env_name:
            self.obs_space_low = list(self.env.env._observation_space.low)
            self.obs_space_high = list(self.env.env._observation_space.high)
        if "locobot" in env_name:
            self.obs_space_low = list(self.env.env._observation_space.low)
            self.obs_space_high = list(self.env.env._observation_space.high)

        self.model = autoregressive_model.AutoRegressiveModel(self.env.observation_space.shape[0], self.env.observation_space.shape[0]*2, size, 3, num_buckets = num_buckets, ac_low = np.array(self.obs_space_low + self.obs_space_low), ac_high = np.array(self.obs_space_high + self.obs_space_high)).cuda()
    ...
    def train_autoregressive_model(self, freq_print = None):
        ...
        for epoch in range(self.num_epochs):
            ...
            loss = -torch.mean(self.model.log_prob(ins, states))
            loss.backward()
            self.optimizer.step()
    ...
    def grid_update_traj(self, traj):
        if "kitchen" in self.env_name or "locobot" in self.env_name:
            return
    ...
    def forward_autoregressive_model(self, s1,s2):
        ...
        with torch.no_grad():
            return self.model.log_prob(ins, vals).detach().cpu().numpy()
    ...
    def plot_comparison(self, curr_state, all_states, total_number_timesteps, thresh, size=20):
        if "pusher" in self.env_name or "kitchen" in self.env_name or "locobot" in self.env_name:
            return
        ...
        r_val = np.exp(np.array(self.forward_autoregressive_model(starts, states)))*total_number_timesteps
        plt.clf()
        plt.cla()
        ...
        r_val_q = np.array([self.query(starts[i], states[i]) for i in range(len(starts))])
        plt.clf()
        plt.cla()
        plt.scatter(states[:, 0], states[:, 1], c=r_val_q, cmap=cm.jet)
        plt.savefig("query_res.png")
        query_res_plt = wandb.Image(plt)

        autoreg_set, r_val = self.get_reachable_set_cut(curr_state, all_states, total_number_timesteps=total_number_timesteps, threshold=thresh)
    ...
    def get_reachable_set_from(self, curr_state, all_states, total_number_timesteps, threshold=0.1):
        starts = np.repeat(curr_state[None], len(all_states), axis=0)
        states = np.array(all_states)
        densities = np.exp(np.array(self.forward_autoregressive_model(starts, states)))*total_number_timesteps
    ...
    def get_reachable_set_cut(self, curr_state, all_states, total_number_timesteps, threshold=5):
        starts = np.repeat(curr_state[None], len(all_states), axis=0)
        states = np.array(all_states)
        densities = np.exp(np.array(self.forward_autoregressive_model(starts, states)))*total_number_timesteps
