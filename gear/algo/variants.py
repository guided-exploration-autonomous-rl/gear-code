from gear.algo import buffer, networks, buffer_human_preferences
from gear.envs.env_utils import DiscretizedActionEnv, DummyWrappedEnv
...
def get_params_vice(env, env_params, discretize=True):
    ...
    if env_params['continuous_action_space']:
        ...
        # policy = default_markov_continuous_policy(env, env_params)
    elif env_params['autoregress']:
        policy = default_autoregressive_policy(env, env_params)
    else:
        if env_params["use_images_in_policy"]:
    ...
    if env_params['use_images_in_reward_model']:
        # TODO: change layers to retireve them from the function
        goal_selector_buffer = buffer.RewardModelBufferImages(**buffer_kwargs, input_image_size=env_params['input_image_size'])
        goal_selector_buffer_validation = buffer.RewardModelBufferImages(**buffer_kwargs, input_image_size=env_params['input_image_size'])
        ...
        goal_selector_buffer = buffer.ViceModelBuffer(**buffer_kwargs)
        goal_selector_buffer_validation = buffer.ViceModelBuffer(**buffer_kwargs)
    ...
    if env_params["use_images_in_policy"] or env_params["use_images_in_reward_model"] or env_params["use_images_in_stopping_criteria"]:
        replay_buffer = buffer.ReplayBufferImages(**buffer_kwargs, input_image_size=env_params['input_image_size'], close_frames=env_params['close_frames'], far_frames=env_params['far_frames'])
        gcsl_kwargs['validation_buffer'] = buffer.ReplayBufferImages(**buffer_kwargs, input_image_size=env_params['input_image_size'], close_frames=env_params['close_frames'], far_frames=env_params['far_frames'])
    else:
        replay_buffer = buffer.ReplayBuffer(**buffer_kwargs)
        gcsl_kwargs['validation_buffer'] = buffer.ReplayBuffer(**buffer_kwargs)
...
def default_autoregressive_policy(env, env_params):
    assert isinstance(env.action_space, Discrete)
    policy_class = networks.AutoRegressiveModel
...
# newly added policy by taking images as inputs
