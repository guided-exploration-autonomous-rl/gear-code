import numpy as np
from gear.envs import goal_env
import gym
...
class GymGoalEnvWrapper(goal_env.GoalEnv):
    ...
    def extract_goal(self, state):
        ...
        Returns:
            obs: A numpy array representing observations
        """
        # print("EXTRACT GOAL", self.obs_dims, self.goal_dims)
    ...
    def sample_goal(self, current_state = None, id = None):
        """
        ...
        goals = self.base_env.sample_goals(1)
        base_obs = self.base_env._get_obs() #self.base_env.reset()
        obs = (10 + self.observation_space.sample()).flatten() # Placeholder - shouldn't actually be used
        goal = goals[desired_key].flatten()
        sgoal = goals[desired_state_key].flatten()
        return np.r_[obs, goal, sgoal]
