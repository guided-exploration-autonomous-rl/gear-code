...
push_configs = {
    ...
    'fixed_start_fixed_goal': dict(
            ...
            fixed_reset=(-0.15,  0.45,-0.15,  0.55), #0.08, 0.53)
            # fixed_goal=(0.2, 0.52, 0.2, 0.55),#-0.25,  0.50),
            fixed_goal=(-0.15, 0.65 , -0.15, 0.7),#-0.25,  0.50),
            fix_reset=True,
            fix_goal=True,
            sample_realistic_goals=True,
            invisible_boundary_wall=True,
...
class SawyerPushAndReachXYEnv(MujocoEnv, Serializable, MultitaskEnv):
    def __init__(
        ...
        assets = 'assets/push_hard.xml'
        self.env_version = env_version
        if env_version == 1:
            assets = 'assets/push_hard_2.xml'
        if self.invisible_boundary_wall:
            model_name = osp.abspath(osp.join(osp.dirname(__file__),assets )) #get_asset_full_path('sawyer_xyz/sawyer_push_and_reach_nips_wall.xml')
        else:
        ...
        self.bimodal_goal = bimodal_goal
        if self.square_puck:
        ...
        self.goal = goal #np.array([fixed_goal, fixed_reset])
        ...
        self.goals = np.array([[0.16, 0.68 ,0.18, 0.68], [-0.15,  0.52,-0.15,  0.5],])
    ...
    def sample_goals(self, batch_size):
        if self.goal is None:
            goals = [self.goals[np.random.randint(len(self.goals))] for i in range(batch_size)][0]
        else:
            goals = self.goals[self.goal]
        goals = {
            'desired_goal': goals,
            'state_desired_goal': goals,
        }
        self.set_goal(goals)
        print("sampling goals", self.goal, goals)
        return goals
...
class SawyerHardPushGoalEnv(GymGoalEnvWrapper):
    def __init__(self, fixed_start=True, goal=None, images=False, image_kwargs=None, env_version=0):
        config_key = 'all'
        fixed_goal = True
        self.goal = goal
        if fixed_start:
        ...
        self.env_version = env_version
        print("Env version start", env_version)
        env = SawyerPushAndReachXYEnv(goal = self.goal, env_version=env_version, **push_configs[config_key])
        self.num_envs = 1
        self.arena_limits = [-0.25, 0.25], [0.48, 0.75]
        if images:
    ...
    def out_of_limit(self, obs):
        if obs[0] < self.arena_limits[0][0] or obs[0] > self.arena_limits[0][1] or obs[1] > self.arena_limits[1][1] or obs[1] < self.arena_limits[1][0]:
            print("finger is out")
            return True
        if obs[2] < self.arena_limits[0][0] or obs[2] > self.arena_limits[0][1] or obs[3] > self.arena_limits[1][1] or obs[3] < self.arena_limits[1][0]:
            print("puck is out")
            return True
        return False  
    ...
    def distance_between_two(self, state1, state2):
        ...
        if state1[0] < subgoal[0][0]:
            if state2[0] < subgoal[0][0]:
                return np.linalg.norm(state1 - state2)
            elif state2[0] < subgoal[1][0]:
                return np.linalg.norm(state1 - subgoal[0]) + np.linalg.norm(subgoal[0]-state2)
            else:
                return np.linalg.norm(state1 - subgoal[0]) + np.linalg.norm(subgoal[0]-subgoal[1]) + np.linalg.norm(subgoal[1] - state2)

        elif state1[0] < subgoal[1][0]:
            if state2[0] < subgoal[1][0]:
                return np.linalg.norm(state1 - state2)
            else:
                return np.linalg.norm(state1 - subgoal[1]) + np.linalg.norm(subgoal[1] - state2)
        else:
            return np.linalg.norm(state1 - state2)
    ...
    def compute_shaped_distance(self, achieved_state, goal):
        if self.env_version == 1:
            bonus = 3
            distance_finger_puck = self.distance_between_two(achieved_state[0:2], achieved_state[2:4])
            distance_puck_goal = self.distance_between_two(achieved_state[2:4], goal[2:4])
            if distance_finger_puck < 0.03:
                return distance_puck_goal
            return distance_finger_puck + bonus
            ...
            bonus = 4
            distance_finger_puck = np.linalg.norm(achieved_state_puck - achieved_state_hand)
            distance_puck_goal = np.linalg.norm(achieved_state_puck - desired_goal_puck)
            if distance_finger_puck < 0.03:
                distance_finger_goal = np.linalg.norm(desired_goal_puck - achieved_state_hand)
                return 3*distance_puck_goal - distance_finger_goal

            return  distance_puck_goal + distance_finger_puck + bonus

    def compute_success(self, achieved_state, goal):        
      return np.linalg.norm(achieved_state[2:4] - goal[2:4]) < 0.05
    ...
    def nearby_goal(self, goal_idx=0, eps=0.1):
        goal = self.base_env.goals[goal_idx]
        
        offset = np.array([np.random.random(), np.random.random(), np.random.random(), np.random.random()])
