...
class UnWrapper(gym.Env):
    def __init__(self, env, goal, max_path_legnth, dense_reward=False):
    ...
    def step(self, a):
        ...
        distance = self._env.compute_shaped_distance(new_state, self.goal)[0]
...
def make_env(env_name, env_params, goal, dense_reward=False, task_config="slide_cabinet,microwave", num_blocks=1, max_path_length=50):
    print(task_config)
    env = envs.create_env(env_name, task_config=task_config, num_blocks=num_blocks)

    wrapped_env, policy, reward_model, replay_buffer, reward_model_buffer, gcsl_kwargs = variants.get_params(env, env_params, discretize=False)
...
def run(wandb_run, continuous_action_space=False, output_dir='/tmp', dense_reward=False, env_name='pointmass_empty', num_blocks=1, num_envs=4,network_layers='128,128',  display_plots=False, save_videos=True, num_tasks=2, task_config='slide_cabinet,microwave', eval_episodes=200, render=False, explore_timesteps=1e4, gpu=0, sample_softmax=False, seed=0, load_rewardmodel=False, batch_size=100, train_regression=False,load_buffer=False, save_buffer=-1, policy_updates_per_step=1,select_best_sample_size=1000, max_path_length=50, hallucinate_policy_freq=5, lr=5e-4, train_with_hallucination=True, start_policy_timesteps=500, log_tensorboard=False, use_oracle=False, comment="", max_timesteps=2e-4, reward_model_name='', **kwargs):
    ...
    fake_env = envs.create_env(env_name, task_config=task_config, num_blocks=num_blocks)
    ...
    model = PPO("MlpPolicy", env, verbose=2, ent_coef = 1e-2, tensorboard_log=f'runs/{wandb_run.id}', policy_kwargs=policy_kwargs)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--object_to_move",type=str, default='slide_cabinet,microwave')
    ...
    wandb_run = wandb.init(
        ...
        'object_to_move':args.object_to_move,
