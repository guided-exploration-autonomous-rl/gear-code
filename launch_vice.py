...
def run( output_dir='/tmp',start_frontier = -1,
    ...
    policy_list = []
    env, policy, goal_selector, classifier_model, replay_buffer, goal_selector_buffer, gcsl_kwargs = variants.get_params_vice(env, env_params)
    env_eval, _, _, _, _, _, _ = variants.get_params_vice(env_eval, env_params)

    policy_list.append(policy)
    for i in range(1):
        _, policy_new, _, _, _, _, _ = variants.get_params_vice(env, env_params)
...
if __name__ == "__main__":
    ...
    wandb.init(project=args.env_name+"gcsl_preferences", name=f"{args.env_name}_vice_{args.seed}", config=params)
